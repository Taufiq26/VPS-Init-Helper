#!/usr/bin/env bash
#
# vps-init-helper — setup-rocky8.sh
#
# Single-file bootstrap for a fresh Rocky Linux 8 (RHEL-family) VPS.
# Sama scope-nya dengan setup.sh (Debian/Ubuntu), diadaptasi untuk dnf,
# firewalld, dan SELinux: nginx, MySQL/PostgreSQL/MongoDB (interaktif),
# Docker, nvm + Node.js LTS + PM2, htop + misc tools, firewalld, fail2ban,
# user sudo baru.
#
# Script ini biasanya dijalankan otomatis oleh setup.sh (yang mendeteksi OS
# lalu delegasi ke sini kalau OS-nya Rocky Linux). Bisa juga dijalankan
# langsung di Rocky 8: sudo bash setup-rocky8.sh
#
# Safe to re-run (idempotent) — sama seperti setup.sh.
#
set -euo pipefail
...
trap 'log_error "Setup gagal di baris $LINENO. Cek pesan error di atas, lalu jalankan ulang setup-rocky8.sh (aman diulang)."' ERR
...
require_root() {
  if [ "$(id -u)" -ne 0 ]; then
    log_error "Script ini harus dijalankan sebagai root. Coba: sudo bash setup-rocky8.sh"
    exit 1
  fi
}
...
guard_os_family() {
  if [ ! -f /etc/os-release ]; then
    log_warn "Tidak bisa mendeteksi OS (/etc/os-release tidak ditemukan). Melanjutkan asumsi Rocky/RHEL-family."
    return 0
  fi
  ...
  case "$OS_ID" in
    rocky)
      if [[ "$OS_VERSION_ID" != 8.* ]]; then
        log_warn "Script ini dites di Rocky Linux 8. Terdeteksi Rocky $OS_VERSION_ID — melanjutkan dengan risiko sendiri."
      else
        log_info "Terdeteksi Rocky Linux $OS_VERSION_ID."
      fi
      ;;
    almalinux|rhel|centos)
      log_warn "Terdeteksi ${PRETTY_NAME:-$OS_ID}. Script ini ditulis untuk Rocky Linux 8, kemungkinan besar tetap jalan di RHEL-family lain (pakai dnf/firewalld yang sama) tapi belum dites khusus."
      ;;
    *)
      log_warn "OS terdeteksi ($OS_ID) bukan Rocky/RHEL-family — script ini pakai dnf/firewalld dan kemungkinan besar akan gagal. Melanjutkan karena dijalankan langsung."
      ;;
  esac
}
...
print_banner() {
  echo -e "${COLOR_STEP}"
  echo "============================================================"
  echo " vps-init-helper — VPS kosong menuju production ready (Rocky Linux 8)"
  echo "============================================================"
  echo -e "${COLOR_RESET}"
}
...
system_update() {
  ...
  dnf_install_if_missing epel-release
  dnf_install_if_missing dnf-plugins-core
  dnf update -y
  dnf_install_if_missing curl wget git unzip gnupg2 ca-certificates policycoreutils-python-utils
  INSTALLED_ITEMS+=("System update + paket dasar (curl, wget, git, unzip, gnupg2, dll) + repo EPEL")
}
...
create_new_user() {
  ...
  while true; do
    read -rp "Masukkan username baru (non-root, akan diberi akses sudo/wheel, kosongkan untuk skip): " username
    if [ -z "$username" ]; then
  ...
  if id "$username" >/dev/null 2>&1; then
    log_info "User '$username' sudah ada di sistem, hanya memastikan grup wheel & SSH key."
  else
    useradd -m -c "" "$username"
  fi
  usermod -aG wheel "$username"

  if ! grep -qE '^\s*%wheel\s+ALL=\(ALL(:ALL)?\)\s+ALL' /etc/sudoers /etc/sudoers.d/* 2>/dev/null; then
    echo '%wheel ALL=(ALL) ALL' > /etc/sudoers.d/99-wheel-sudo
    chmod 440 /etc/sudoers.d/99-wheel-sudo
    log_info "Grup wheel belum punya akses sudo penuh di sudoers default, ditambahkan lewat /etc/sudoers.d/99-wheel-sudo."
  fi
  ...
  INSTALLED_ITEMS+=("User sudo baru: $NEW_USER (SSH key terpasang, grup wheel)")
}
...
setup_firewall() {
  log_step "Konfigurasi firewalld"
  dnf_install_if_missing firewalld
  ...
  INSTALLED_ITEMS+=("firewalld aktif — port terbuka: $SSH_PORT (SSH), 80 (HTTP), 443 (HTTPS)")
  log_info "firewalld aktif. Port terbuka: $SSH_PORT, 80, 443."
...
apply_cloudflare_ip_lock() {
  log_step "Membatasi port 80/443 hanya dari IP Cloudflare (firewalld rich rules)"
  local cf_v4 cf_v6
  ...
  mkdir -p "$STATE_DIR"
  echo "$cf_v4" > "$STATE_DIR/cloudflare-ips-v4.txt"
  echo "$cf_v6" > "$STATE_DIR/cloudflare-ips-v6.txt"

  cat > /etc/cron.weekly/vps-init-helper-cloudflare-refresh <<CRON_EOF
#!/usr/bin/env bash
mkdir -p "$OUTPUT_DIR/logs"
$SCRIPTS_DIR/refresh-cloudflare-ips.sh >> "$OUTPUT_DIR/logs/cloudflare-refresh.log" 2>&1
CRON_EOF
  chmod +x /etc/cron.weekly/vps-init-helper-cloudflare-refresh

  INSTALLED_ITEMS+=("firewalld dibatasi — port 80/443 hanya menerima koneksi dari IP Cloudflare (auto-refresh mingguan aktif)")
  log_info "Port 80/443 sekarang hanya menerima koneksi dari IP Cloudflare. Daftar IP akan di-refresh otomatis tiap minggu."
  return 0
}

setup_fail2ban() {
  log_step "Konfigurasi fail2ban"
  dnf_install_if_missing fail2ban fail2ban-firewalld
...
F2B_EOF
  ...
  {
    echo "[DEFAULT]"
    echo "banaction = firewallcmd-rich-rules"
    echo "banaction_allports = firewallcmd-allports"
    echo "bantime = 1h"
    echo "findtime = 10m"
    echo "maxretry = 5"
    if [ "$TELEGRAM_ENABLED" = "yes" ]; then
  ...
  INSTALLED_ITEMS+=("fail2ban — jail sshd aktif (port $SSH_PORT, maxretry 5, bantime 1h, integrasi firewalld)${f2b_note}")
}

install_nginx() {
  log_step "Install nginx"
  dnf_install_if_missing nginx
  systemctl enable --now nginx >/dev/null 2>&1

  if command_exists getenforce && [ "$(getenforce)" = "Enforcing" ]; then
    setsebool -P httpd_can_network_connect 1
    log_info "SELinux: httpd_can_network_connect diaktifkan (wajib supaya nginx bisa reverse-proxy ke app Node.js/PM2 di localhost)."
  fi

  local ver
  ver=$(nginx -v 2>&1 | grep -oP '[0-9.]+' | head -1 || echo "?")
  INSTALLED_ITEMS+=("nginx $ver (SELinux: httpd_can_network_connect diaktifkan untuk reverse proxy)")
}

install_mysql() {
  if is_done "mysql_install"; then
    log_info "MariaDB sudah pernah di-setup sebelumnya, lewati."
    INSTALLED_ITEMS+=("MariaDB (sudah ada sebelumnya)")
    systemctl is-active --quiet mariadb && MYSQL_UNIT="mariadb"
    return 0
  fi
  log_step "Install MariaDB"
  dnf_install_if_missing mariadb-server
  systemctl enable --now mariadb >/dev/null 2>&1
  MYSQL_UNIT="mariadb"

  local my_cnf="/etc/my.cnf.d/mariadb-server.cnf"
  if [ -f "$my_cnf" ] && ! grep -q '^bind-address' "$my_cnf"; then
    sed -i '/^\[mysqld\]/a bind-address = 127.0.0.1' "$my_cnf"
    systemctl restart mariadb
    log_info "MariaDB: bind-address dipaksa ke 127.0.0.1 (default paket Rocky belum membatasi ini)."
  fi

  local mysql_root_password
  prompt_password_confirmed mysql_root_password "Password root MariaDB baru"
  ...
  mysql -u root <<SQL 2>/dev/null || log_warn "Sebagian perintah MariaDB secure-setup gagal (kemungkinan sudah pernah dijalankan sebelumnya)."
...
SQL
  ...
  INSTALLED_ITEMS+=("MariaDB — root password sudah diset (SIMPAN sendiri, tidak disimpan script ini)")
  log_info "MariaDB root password sudah diset. SIMPAN baik-baik, script ini tidak menyimpannya."
  return 0
}
...
install_postgresql() {
  ...
  dnf_install_if_missing postgresql-server postgresql-contrib
  ...
  if [ -f "$pg_hba" ] && grep -qE '^\s*host\s+all\s+all\s+127\.0\.0\.1/32\s+ident' "$pg_hba"; then
    sed -i -E 's/^(\s*host\s+all\s+all\s+127\.0\.0\.1\/32\s+)ident/\1scram-sha-256/' "$pg_hba"
    sed -i -E 's/^(\s*host\s+all\s+all\s+::1\/128\s+)ident/\1scram-sha-256/' "$pg_hba"
    log_info "pg_hba.conf: default 'ident' untuk koneksi localhost diganti ke 'scram-sha-256' (supaya login password bisa dipakai app)."
...
MINIO_UNIT_EOF
  ...
  INSTALLED_ITEMS+=("MinIO (AGPLv3) — API port 9000, Console port 9001, root user: ${minio_user} (password SIMPAN sendiri, tidak disimpan script ini). Port belum dibuka di firewalld (default localhost-only, lihat security.md).")
  log_info "MinIO aktif (systemd service 'minio'). Console: http://<ip-server>:9001 — port belum dibuka firewalld, lihat security.md."
  return 0
}
...
select_and_install_databases() {
  ...
  for db in "${SELECTED_DBS[@]}"; do
    case "$db" in
      mysql)      if ! install_mysql; then log_warn "Instalasi MariaDB gagal, lanjut ke langkah berikutnya."; fi ;;
...
install_docker() {
  ...
  dnf_install_if_missing dnf-plugins-core
  # Docker tidak menerbitkan repo khusus "rocky", tapi repo "centos" resmi
  # kompatibel penuh (Rocky adalah rebuild RHEL) — ini pola standar yang
  # dipakai banyak panduan Rocky/Alma.
  if ! dnf config-manager --add-repo https://download.docker.com/linux/centos/docker-ce.repo; then
    log_warn "Gagal menambahkan repo Docker. Melewati instalasi Docker."
    return 1
  fi
...
install_misc_tools() {
  log_step "Install tools tambahan (htop, tmux, ncdu, net-tools, dnf-automatic)"
  dnf_install_if_missing htop tmux ncdu net-tools dnf-automatic
  sed -i 's/^apply_updates.*/apply_updates = yes/' /etc/dnf/automatic.conf
  systemctl enable --now dnf-automatic.timer >/dev/null 2>&1
  INSTALLED_ITEMS+=("htop, tmux, ncdu, net-tools")
  INSTALLED_ITEMS+=("dnf-automatic (auto security updates, setara unattended-upgrades)")
}
...
install_monit() {
  ...
  dnf_install_if_missing monit

  local conf="/etc/monit.d/vps-init-helper.conf"
  mkdir -p /etc/monit.d
  {
    echo "set daemon 120"
    ...
    if [ -n "$MYSQL_UNIT" ]; then
      monit_process_block "mariadb" "mysqld" "$MYSQL_UNIT"
    fi
    if is_done "postgresql_install"; then
  ...
  if ! grep -q "^include /etc/monit.d" /etc/monitrc 2>/dev/null; then
...
install_logwatch() {
  ...
  dnf_install_if_missing logwatch
  mkdir -p "$OUTPUT_DIR/logs"
...
if command -v dpkg >/dev/null 2>&1; then
  ...
  rpm -qa > "$WORKDIR/installed-packages.txt"
fi
...
RESTORE_SCRIPT_EOF
  ...
  cat > "$SCRIPTS_DIR/refresh-cloudflare-ips.sh" <<'CFREFRESH_SCRIPT_EOF'
#!/usr/bin/env bash
...
CFREFRESH_SCRIPT_EOF

  chmod +x "$SCRIPTS_DIR/backup.sh" "$SCRIPTS_DIR/restore.sh" "$SCRIPTS_DIR/refresh-cloudflare-ips.sh"
}
...
generate_report() {
  ...
  mkdir -p "$OUTPUT_DIR"
  {
    ...
    echo "- firewalld dibatasi ke IP Cloudflare saja (port 80/443): $( [ "$CF_IP_LOCK" = "yes" ] && echo "ya" || echo "tidak" )"
    echo "- SELinux: $(command_exists getenforce && getenforce || echo "tidak diketahui") (dibiarkan enforcing — jangan dimatikan)"
    echo
    echo "## Lokasi penting"
    echo
...
# Panduan Hardening Keamanan — vps-init-helper (Rocky Linux 8)
...
$( if [ -n "$NEW_USER" ]; then echo "User sudo yang dibuat: \`$NEW_USER\`"; else echo "**User sudo baru TIDAK dibuat** (pembuatan user di-skip saat setup)."; fi )
...
Kamu memilih SKIP pembuatan user baru saat setup, jadi setup-rocky8.sh TIDAK
...
- User non-root lain di server ini dengan SSH key + akses sudo/wheel, ATAU
...
Mengurangi noise dari bot yang scan port 22. Di Rocky/RHEL, SELinux akan
**menolak sshd bind ke port baru** kecuali port itu dilabeli dulu — beda
dengan Debian/Ubuntu. WAJIB jalankan \`semanage\` ini SEBELUM restart sshd:

\`\`\`
sudo semanage port -a -t ssh_port_t -p tcp <port-baru>
\`\`\`

(paket \`policycoreutils-python-utils\` yang menyediakan \`semanage\` sudah
terinstall oleh setup-rocky8.sh). Baru edit \`/etc/ssh/sshd_config\`:

\`\`\`
Port <port-baru>
\`\`\`

Lalu buka port baru di firewalld SEBELUM restart SSH (supaya tidak ter-lockout):

\`\`\`
sudo firewall-cmd --permanent --add-port=<port-baru>/tcp
...
sudo firewall-cmd --permanent --remove-port=$SSH_PORT/tcp 2>/dev/null || true
sudo firewall-cmd --reload
\`\`\`

## 3. Batasi akses database ke localhost saja

Database yang di-install setup-rocky8.sh secara default sudah dipaksa listen
di \`127.0.0.1\` saja. Kalau kamu memang butuh akses remote ke database, baru:

- **MariaDB**: edit \`bind-address\` di \`/etc/my.cnf.d/mariadb-server.cnf\`,
  lalu buka port dengan \`sudo firewall-cmd --permanent --add-rich-rule='rule family="ipv4" source address="<ip-trusted>" port port="3306" protocol="tcp" accept'\`
  (JANGAN buka ke semua IP).
- **PostgreSQL**: edit \`listen_addresses\` di \`/var/lib/pgsql/data/postgresql.conf\`
  dan tambahkan rule di \`/var/lib/pgsql/data/pg_hba.conf\`, lalu buka port
...
- **MinIO**: MinIO sendiri listen di semua interface (\`0.0.0.0:9000\`/\`9001\`),
  ...
  luar: \`sudo firewall-cmd --permanent --add-rich-rule='rule family="ipv4" source address="<ip-trusted>" port port="9000" protocol="tcp" accept'\`
  (ulangi untuk port 9001; JANGAN buka ke semua IP kalau bisa dihindari, dan
  kalau memang harus publik, taruh nginx reverse proxy + TLS di depannya).
...
## 4. Review firewalld, fail2ban & SELinux
...
Pastikan hanya port yang benar-benar perlu yang muncul di \`firewall-cmd --list-all\`,
dan SELinux tetap \`Enforcing\` (JANGAN di-set ke \`permissive\`/\`disabled\` — kalau
ada masalah "permission denied" yang aneh dari service tertentu, cari tahu dulu
lewat \`sudo ausearch -m avc -ts recent\` dan perbaiki policy-nya, bukan matikan
SELinux-nya).

## 5. Cek dnf-automatic aktif

\`\`\`
sudo systemctl status dnf-automatic.timer
cat /etc/dnf/automatic.conf | grep apply_updates
\`\`\`

## 6. Kalau server ini menghadap publik dengan domain

- Pasang SSL lewat Let's Encrypt: \`sudo dnf install -y certbot python3-certbot-nginx\`
...
- firewalld di server ini SUDAH dibatasi hanya menerima port 80/443 dari IP
  resmi Cloudflare (diaktifkan saat setup). Origin server tidak bisa diakses
...
- Port 80/443 di server ini MASIH terbuka untuk semua IP (bukan hanya
  ...
  mengambil https://www.cloudflare.com/ips-v4 dan ips-v6 lalu buat
  firewalld rich-rule untuk tiap range.
CFNOTE2
)
...
$( [ "$AIDE_INSTALLED" = "yes" ] && echo "- AIDE aktif — baseline file integrity sudah dibuat. Kalau kamu SENGAJA mengubah file sistem (upgrade, config baru), jalankan \`sudo aide --init\` ulang dan salin hasilnya ke \`/var/lib/aide/aide.db.gz\` supaya baseline tidak terus memicu alert palsu." )
...
`backup.sh`/`restore.sh` sendiri tidak bergantung pada OS tertentu (cuma
memanggil `mysqldump`/`pg_dumpall`/`mongodump` kalau ada), jadi tetap bisa
dipakai untuk migrasi dari VPS Debian/Ubuntu ke Rocky atau sebaliknya, selama
engine database yang sama tersedia di server tujuan.
...
Jalankan `setup.sh` yang sama seperti pertama kali dulu. Script ini akan
mendeteksi OS VPS baru secara otomatis (Debian/Ubuntu atau Rocky Linux 8) dan
menjalankan alur instalasi yang sesuai — kamu tidak perlu memilih manual:
...
- `systemctl status nginx mariadb mysql postgresql mongod docker` (sesuai yang di-install)
...
" ;;
...

  {
    ...
    echo "- Firewall (firewalld) aktif dengan kebijakan default menolak semua koneksi masuk kecuali yang diizinkan eksplisit"
    echo "- SELinux aktif (enforcing) sebagai lapisan pertahanan tambahan di level kernel"
...
<ul>
  ...
  <li>Firewall (firewalld) aktif dengan kebijakan default menolak semua koneksi masuk kecuali yang diizinkan eksplisit</li>
  <li>SELinux aktif (enforcing) sebagai lapisan pertahanan tambahan di level kernel</li>
